...
#    print("check the MINK data type: ", type(MINK))
    #print("check the MINK: ", MINK[2,1002:1010])
...
Yi=cupy.sparse.csr_matrix(cp.ones((600,775569)),dtype='float32')
L=cupy.sparse.csr_matrix(cp.empty((600,775569)),dtype='float32')
...
for i in range(l0,l1):
    ...
    # because the old version of cupy used here, so matrix multiplication * between two matrices, which is equal to @ in the later version of cupy or numpy (tested in testcupy.py file)
    ...
    #L = A * AdjMatrix  # better to use cupy for matrix multiplication since cupy has sparse matrix functionality that saves more memory, otherwise CUDA is good to be used here shown as the above
